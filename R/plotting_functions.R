...
plot_regressions <- function(deggs_object,
  ...
  # prepare data frame
  df <- data.frame(t(assayData[gene_A, ]),
    t(assayData[gene_B, ]),
    metadata,
    check.names = FALSE
  )
  colnames(df)[3] <- "category"
...
node_boxplot <- function(gene,
  ...
  x <- deggs_object[["metadata"]]
  y <- as.numeric(deggs_object[["assayData"]][[assayDataName]][gene, ])
  col <- viridis::viridis(n = nlevels(deggs_object[["metadata"]]))
  cols <- col[deggs_object[["metadata"]]]
...
#' @export
View_diffNetworks <- function(deggs_object, host = NULL, port = NULL) {
  ...
  server <- function(input, output, session) {
    outVar = shiny::reactive({
      if (is.data.frame(category_networks[[input$category]])) (
        category_networks[[input$category]]
      ) else (
    ...
    output$network <- visNetwork::renderVisNetwork({
      if (is.data.frame(category_networks[[input$category]])) {
        ...
        if (multiOmic) {
          ...
          col <- c("royalblue", "gray60")
          edges$color <- ifelse(edges[, sig_var] < 0.05, "royalblue", "gray60")
        ...
        if (nrow(edges) == 0) {
          ...
          network.title <- paste0("No differential interaction with ", sig_var,
                                  "<", input$slider, ".<br> Try to increase the ", sig_var, " threshold.")
      ...
      if (is.null(input$current_nodes_selection) &
        ...
        plot_regressions(
          ...
          assayDataName = ifelse(multiOmic, 
                                 edges[input$current_edges_selection, "layer"],
                                 1),
    ...
    shiny::sidebarLayout(
      shiny::sidebarPanel(
        ...
        shiny::sliderInput("slider",
                           label = ifelse(deggs_object[["use_qvalues"]], "q values", "p values"),
                           min = 0.01,
